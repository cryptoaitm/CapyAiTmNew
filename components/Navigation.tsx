...
import { game, rankingOne, tonWallet } from '@/images';
...
const navItems: NavItem[] = [
    ...
    { name: 'Rank', image: rankingOne, view: 'rank' }, 
    { name: 'Friends', icon: Friends, view: 'friends' },
    { name: 'Earn', icon: Coins, view: 'earn' },
    { name: 'Airdrop', image: tonWallet, view: 'airdrop' }, 
];
...
export default function Navigation({ currentView, setCurrentView }: NavigationProps) {
    ...
    return (
        <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-[calc(100%-2rem)] w-full max-w-xl bg-[#000000] flex justify-around items-center z-40 text-xs border-t border-[#43433b] max-h-24">
            {navItems.map((item) => (
                <button
                  key={item.name}
                  ...
                  className="flex-1"
                >
                    <div className={`flex flex-col items-center justify-center ${currentView === item.view ? 'text-white bg-[#1c1f24]' : 'text-[#fafafa]'} h-16 m-1 p-2 rounded-2xl`}>
